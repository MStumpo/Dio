...
void Network::AdjMatrix::updateAdj() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            ...
            data[i][j]->value += parent.hp.lr*(data[i][j]->destination->value * data[i][j]->U + data[i][j]->sender->value*(1-data[i][j]->destination->trace) - parent.hp.reg*data[i][j]->value*C[i][j] );
